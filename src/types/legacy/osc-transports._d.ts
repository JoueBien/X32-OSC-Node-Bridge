...
> extends EventEmitter {
  ...
  on(
    event: "message",
    listener: (message: RM, timeTag: FullTimeTag, info: any) => unknown
  ): this
  on(
    event: "bundle",
    listener: (
      message: ReadBundle<RA, RM>,
      timeTag: FullTimeTag,
      info: any
    ) => unknown
  ): this
...
export interface PortConstructor<Additional = never, Options = never> {
  new (
    options?: { unpackSingleArgs?: false; metadata?: false } & Options
  ): Port<
    Argument,
    Message<Argument>,
  ...
  > &
    Additional
  new (
    options: { unpackSingleArgs?: false; metadata: true } & Options
  ): Port<
    ReadArgumentWithMetadata,
  ...
  > &
    Additional
  new (
    options: { unpackSingleArgs: true; metadata?: false } & Options
  ): Port<
    Argument,
    SingleByteMessage<Argument>,
    Argument | WriteArgumentWithMetadata,
  ...
  > &
    Additional
  new (
    options: { unpackSingleArgs: true; metadata: true } & Options
  ): Port<
    ReadArgumentWithMetadata,
...
export interface OSCTransport {
  ...
  Relay: new (
    port1: Port<any, any, any, any>,
    port2: Port<any, any, any, any>,
    options?: { raw?: boolean }
  ) => Relay
  SerialPort: PortConstructor<
    PortGenericOperations & { sendRaw(encoded: any): void },
    SerialPortOptions
  >
  UDPPort: PortConstructor<
    PortGenericOperations & {
      sendRaw(encoded: any, address: any, port: any): void
    },
    UDPPortOptions
  >
  TCPSocketPort: PortConstructor<
    PortGenericOperations & { sendRaw(encoded: any): void },
    TCPSocketPortOptions
  >
  WebSocketPort: PortConstructor<
    PortGenericOperations & { sendRaw(encoded: any): void },
